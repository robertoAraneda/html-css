...
function editRow(id){
	...
	.then(({data}) => {
		...
		const typeCrewMember = document.getElementById('type-crew-member').value = data.type_crew_member
		const specimenAt = document.getElementById('specimen-at').value = transformDate(data.specimen_at)
